    ...
    if [ "$SKIP_TORCH" = true ]; then
        ...
        if [ -n "$_radeon_url" ]; then
            ...
            if [ "$_radeon_listing_ok" = true ]; then
                # 1. First pass: Pick the absolute highest version wheel for each package
                ...
                # Helper to DRY up the URL-decoding and version extraction
                _extract_version() {
                    local whl=$1
                    local pkg=$2
                    if [ -n "$whl" ]; then
                ...
                if [ -n "$_torch_ver" ] && [ -n "$_tv_ver" ] && [ -n "$_ta_ver" ]; then
                    ...
                    # Normalize minor versions to their torch-equivalent (tv 0.24 -> torch 2.9)
                    _tv_equiv_minor=$((_tv_minor - 15))
                    _ta_equiv_minor=$_ta_minor

                    # 2. Downpairing Logic: Find the lowest common minor version among the three
                    _target_minor=$_torch_minor
                    if [ "$_tv_equiv_minor" -lt "$_target_minor" ]; then
                        _target_minor=$_tv_equiv_minor
                    fi
                    if [ "$_ta_equiv_minor" -lt "$_target_minor" ]; then
                        _target_minor=$_ta_equiv_minor
                    fi

                    # If any package overshot the target minor version, re-pick specifically for the target
                    _repick_needed=false
                    ...
                    # If we had to downpair, re-parse the major/minor vars from the newly fetched wheels
                    if [ "$_repick_needed" = true ] && [ -n "$_torch_ver" ] && [ -n "$_tv_ver" ] && [ -n "$_ta_ver" ]; then
                        _torch_major=${_torch_ver%%.*}
                        _torch_minor=${_torch_ver#*.}
                        _ta_major=${_ta_ver%%.*}
                        _ta_minor=${_ta_ver#*.}
                ...
                # 4. Fallback execution if the matched set is entirely missing
